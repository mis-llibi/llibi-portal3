...
class DeelUploadEnrollee implements ToCollection, WithHeadingRow, WithBatchInserts
{
    /**
    ...
    private function importToMasterlist($row)
    {
        if (trim(!empty($row['employee_id']))) {
            ...
            $exist = (new ManageDeelClients)->checkIfExistingPrincipal($row['employee_id'], $birthDate);

            if (!$exist) {

                //$newRel = ($row['relation'] == 'EMPLOYEE' ? 'PRINCIPAL' : $row['relation']);
