    ...
    <>
      ...
      <div className="lg:flex">
        <div className="basis-42">
          ...
          <Button
            onClick={() => {
              if (props?.fields.length === 2) return

              props?.append({})
            }}
            className={`${
              props?.fields.length === 2 &&
              'bg-gray-700 hover:bg-gray-700 active:bg-gray-700'
            } cursor-pointer text-sm normal-case`}>
            {props?.fields.length === 2 ? 'Max 2 Dependents' : 'Add Dependent'}
          </Button>
        </div>
      </div>
    </>
