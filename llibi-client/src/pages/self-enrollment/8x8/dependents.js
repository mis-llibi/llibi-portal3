...
import options from '@/hooks/self-enrollment/EigthByEigthOptions'
...
const Dependents = () => {
    ...
    useEffect(() => {
        ...
        setValue('milestone', client?.principal[0]?.milestone)
        ...
        if (client?.principal[0]?.form_locked == 1) {
            if (!client?.principal[0]?.milestone) setMilestone(true)
            //window.location.pathname = `/self-enrollment/8x8/form-locked`
        } else {
            if (client?.principal.length > 0)
                if (client?.principal[0]?.status == 1) {
                    window.location.pathname = `/self-enrollment/8x8/`
                } else {
                    setPage(true)
                }
        }
    ...
    useEffect(() => {
        const data = {
            ...
            memberId: client?.principal[0]?.member_id,
        }
        if (milestone)
            Swal.fire({
                title: '<strong><u>Milestone Enrollment</u></strong>',
                icon: 'info',
                html:
                    '<div style="text-align:center;font-size:22px;">You can change your civil status or add your new born baby here.</div>',
                showConfirmButton:
                    client?.principal[0]?.civil_status != 'SINGLE',
                showDenyButton: true,
                confirmButtonText: 'Add New Born Baby',
                denyButtonText: 'Change Civil Status',
    ...
    return (
        <>
            <GuestLayout title="8x8 Dependent Self-Enrollment Portal">
                ...
                <form encType="multipart/form-data">
                    ...
                    <input type="hidden" {...register('milestone')} />
                    ...
                    <div className="flex gap-2">
                        <div>
                            {/* client logo */}
                            <img
                                src={`${basePath}/self-enrollment/demoimage.png`}
                                width={150}
                            />
                        </div>
                    ...
                    <div className="mt-3 p-4 rounded-md bg-gray-100">
                        ...
                        <div className="grid md:grid-cols-2 lg:flex gap-2 text-sm truncate">
                            ...
                            <div className="pr-2 flex md:flex-none lg:flex">
                                ...
                                <p className="ml-2 font-bold text-red-400">
                                    ₱{' '}
                                    {client?.principal[0]?.mbl?.toLocaleString(
                                        'en-US',
                                    ) || 'N/A'}
                                </p>
                            </div>
                ...
                <div className="float-right">
                    ...
                    <Button
                        ...
                        className={`normal-case bg-purple-400 hover:bg-purple-700 focus:bg-purple-900 active:bg-purple-500 ring-purple-200 p-2 mb-2 md:mt-0 md:mr-2`}
                    ...
                    <Button
                        ...
                        className={`normal-case bg-orange-400 hover:bg-orange-700 focus:bg-orange-900 active:bg-orange-500 ring-orange-200 p-2 ${
                            watch('milestone') == 1 && 'hidden'
                        }`}
                        onClick={() =>
