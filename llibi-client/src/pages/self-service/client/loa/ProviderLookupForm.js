...
const ProviderLookupForm = ({
  ...
  const handleSetSendLoaToProvider = () => {
    Swal.fire({
      title:
        'SEND REQUEST LOA TO PROVIDER',
      icon: 'warning',
      allowOutsideClick: false,
      html:
        '<p style="text-align:left;">Do you want to send your request loa directly to your provider?</p>',
      showCloseButton: true,
      showCancelButton: true,
      confirmButtonText: 'Yes, accept it',
      cancelButtonText: 'No, cancel',
      ...
      if (result.isConfirmed) {
        setSendLoadToProvider(1)
      } else {
        setSendLoadToProvider(0)
      }
    })
  }
    ...
    <form onSubmit={handleSubmit(onChooseProvider)} className="px-2">
      ...
      <div
        ...
        <div className="basis-1/2">
          ...
          <div
            className={`h-96 w-full bg-gray-50 mt-2 rounded-md overflow-y-auto ${
              (!hospital || hosploading) && 'flex items-center justify-center'
            }`}>
            {hosploading ? (
        ...
        <div
          ...
          <div
            className={`h-96 w-full bg-gray-50 mt-2 rounded-md overflow-y-auto ${
              (!doctor || docloading) && 'flex items-center justify-center'
            }`}>
            {docloading ? (
              <SyncLoader color="#CDCDCD" margin={10} size={15} />
            ) : doctor ? (
              ...
              <span className="text-orange-600 font-semibold text-sm">
                No doctor found, please search the name of the doctor
              </span>
            )}
          </div>
        </div>
